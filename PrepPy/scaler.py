def scaler(X_train, X_validation, X_test, colnames):
    ...
    -------
    dict
      Stores the scaled and transformed X_train and X_test sets
      separately as two dataframes.

    Examples
    --------
    >>> from PrepPy import prepPy as pp
    >>> x_train = pd.DataFrame(np.array([['Blue', 56, 4],
    ['Red', 35, 6], ['Green', 18, 9]]),
    columns=['color', 'count', 'usage'])

    >>> x_test = pd.DataFrame(np.array([['Blue', 66, 6],
    ['Red', 42, 8], ['Green', 96, 0]]),
    columns=['color', 'count', 'usage'])

    >>> X_validation = pd.DataFrame(np.array([['Blue', 30, 18],
    ['Red', 47, 2], ['Green', 100, 4]]),
    columns=['color', 'count', 'usage'])

    >>> colnames = ['count', 'usage']

    >>> X_train = pp.scaler(x_train, X_validation, x_test,
    colnames)['x_train']
    ...
    >>> X_test = pp.scaler(x_train, X_validation,
    x_test, colnames)['x_test']
    ...
    The type of X_train passed is: {}. Please convert to\
        dataframe'.format(type(X_train)))

    if not isinstance(X_test, pd.DataFrame):
        raise Exception('X_test is not a dataframe.\
            The type of X_test passed is: {}.\
                Please convert to\
                    dataframe'.format(type(X_test)))

    if not isinstance(X_validation, pd.DataFrame):
        raise Exception('X_validation is not a dataframe.\
            The type of X_validation passed is: {}.\
                Please convert to\
    ...
    if X_train.empty is True:
        raise Exception('X_train cannot be empty')

    if X_test.empty is True:
